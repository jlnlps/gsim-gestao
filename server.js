...
app.get('/api/financeiro/balanco', async (req,res) => { const {data_inicio,data_fim}=req.query; const er=await get('SELECT COALESCE(SUM(CASE WHEN sinal_pago=1 THEN valor_sinal ELSE 0 END)+SUM(CASE WHEN saldo_pago=1 THEN (valor_total-valor_sinal) ELSE 0 END),0) as total FROM pedidos WHERE data_entrada BETWEEN ? AND ?', [data_inicio,data_fim]); const ar=await get("SELECT COALESCE(SUM(CASE WHEN sinal_pago=0 THEN valor_sinal ELSE 0 END)+SUM(CASE WHEN saldo_pago=0 THEN (valor_total-valor_sinal) ELSE 0 END),0) as total FROM pedidos WHERE status NOT IN ('Cancelado','Finalizado') AND data_entrega BETWEEN ? AND ?", [data_inicio,data_fim]); const dfp=await get('SELECT COALESCE(SUM(valor),0) as total FROM despesas_fixas_lancamentos WHERE pago=1 AND data_vencimento BETWEEN ? AND ?', [data_inicio,data_fim]); const dvp=await get('SELECT COALESCE(SUM(valor),0) as total FROM despesas_variaveis WHERE pago=1 AND data_despesa BETWEEN ? AND ?', [data_inicio,data_fim]); const dfpend=await get('SELECT COALESCE(SUM(valor),0) as total FROM despesas_fixas_lancamentos WHERE pago=0 AND data_vencimento BETWEEN ? AND ?', [data_inicio,data_fim]); const dvpend=await get('SELECT COALESCE(SUM(valor),0) as total FROM despesas_variaveis WHERE pago=0 AND data_vencimento BETWEEN ? AND ?', [data_inicio,data_fim]); const cm=await get('SELECT COALESCE(SUM(em.quantidade*em.custo_unitario),0) as total FROM estoque_movimentos em JOIN pedidos p ON em.pedido_id=p.id WHERE em.tipo='saida' AND p.data_entrada BETWEEN ? AND ?', [data_inicio,data_fim]); const total_desp=dfp.total+dvp.total+cm.total; res.json({entradas_recebidas:er.total,a_receber:ar.total,despesas_pagas:total_desp,despesas_fixas_pagas:dfp.total,despesas_variaveis_pagas:dvp.total,custo_materiais:cm.total,a_pagar:dfpend.total+dvpend.total,lucro_liquido:er.total-total_desp}); });
